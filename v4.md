...
- (Coming Soon) Quickly access your filters with [Advanced Indicators](#advanced-indicators-coming-soon)
- (Coming Soon) Sort by multiple table columns with [Multi Sort](#advanced-indicators-coming-soon)
...
There are the current known issues in v4 beta that are being resolved. Some of these issues are Filament v4 issues that will resolve themselves as Filament is updated.

- Displaying View Manager as slideOver - [Issue #17024](https://github.com/filamentphp/filament/issues/17024)
    - When displaying the [View Manager as a slideOver](#displaying-as-a-slideover) the quick save form is displayed and the action group dropdown is incorrectly located due to an alpine.js error. 
    - Workaround: Disable this functionality `->viewManagerSlideOver(false)` so it is displayed as a dropdown.
- Deferring Filters - [Issue #16916](https://github.com/filamentphp/filament/issues/16916)
    - Filament now defers filters by default. This is causing issues with Advanced Filters
    - Workaround: Disable Filament's defer filters functionality by adding `->deferFilters(false)` to your tables or by disabling it [globally](https://filamentphp.com/docs/4.x/tables/overview#global-settings). 

I will update this section as known issues are discovered and/or solved.

### Requirements

Advanced Tables requires `PHP 8.2+`, `MySQL 5.7.8+` or `Postgres`, `Filament 4.0.0 beta-6+`, and `Laravel 11.28+`.
...
### Setting a default table multi-sort (Coming Soon)

> Note: This feature will be coming to v4 in the near future.

If [Multi-Sort](#multi-sort-coming-soon) is enabled, you may multi-sort your preset views through the same `defaultSort()` method. Just pass an array of columns and their sort direction to `defaultSort()`:
...
## Multi-Sort (Coming Soon)

![Multi-Sort](https://advancedtables.com/images/multi-sort.png)

> Note: This feature will be coming to v4 in the near future.

Advanced Tables will soon allows your users to sort their tables by multiple columns. Using the new Multi-Sort dropdown, users can add additional columns to sort by, easily change sort direction, and even reorder the columns. And Multi-Sort is completely integrated with [Preset Views](#preset-views) and [User Views](#user-views).

## Advanced Indicators (Coming Soon)

![Advanced indicators](https://advancedtables.com/images/advanced-indicators.png)

> Note: This feature will be coming to v4 in the near future. It is currently available to test in the 3.10 beta.
